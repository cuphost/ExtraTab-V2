...
require("FrameWork")
Execute_Sudo = {362270285,159127730,446287802}
-------- Source Body --------
...
local random_sgp = ExtraTab:match('فوروارد (%d+)')
local FwdLimit = redis:ttl("FwdLimit:"..BOTID) or -2
         ...
         if tonumber(msg.reply_to_message_id) > 0 then
          ...
          local ExtraTab = 'این پیام به تعداد گروه '..random_sgp..' فوروارد شد •'
             ...
             SendMessage(msg.chat_id, '• پیام ریپلی نشده است •', 'md')
		 end
		 else
		     SendMessage(msg.chat_id, '• محدودیت زمانی فعال است لطفا بعدا تلاش کنید •', 'md')
		 end
end
---------- Add ----------
if ExtraTab and ExtraTab:match('^( ادد) (%d+) (%d+)') and is_sudo(msg) then
local match = {
        ExtraTab:match("^(ادد) (%d+) (%d+)")
      }
local AddLimit = redis:ttl("AddLimit:"..BOTID) or -2
         if AddLimit == -2 then
          ...
          local ExtraTab = '• فرد با شناسه '..match[3]..' به تعداد گروه '..match[2]..' اضافه شد •'
             SendMessage(msg.chat_id, ExtraTab, 'md')
...
----------- Reset -----------
if ExtraTab == 'ریست' and is_sudo(msg) then
...
----------- Join Stats -----------
if ExtraTab == 'عضویت فعال' and is_sudo(msg) then
        if JoinStats == 'no' then
...
if ExtraTab == 'عضویت غیرفعال' and is_sudo(msg)then
...
----------- Reload -----------
if ExtraTab == 'ریستارت' and is_sudo(msg) then
        dofile('./ExtraTab.lua')
...
redis:setex('JoinLimit:'..BOTID,120,true)
end
end
end
-------------------------
